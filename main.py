...
introduction = 'The goal of this project is to identify the instrument being played from a given snippet of audio.  We will be using an established dataset of audio files such as NSynth to train a convolutional neural network to fulfill this goal. NSynth already has its audio files encoded down to integer arrays, and comes with a wide featureset that we will be able to use for training. We will be using libraries such as scikit-learn library to train and test our model, as well as Librosa to extract features from the dataset if we need to. We will be using the matplotlib library to visualize our data.'
