...
introduction = 'The goal of this project is to identify the instrument being played from a given snippet of audio. This concept has been explored in several research articles as part of bigger audio projects, such as clustering of instrument catagories from a multi-instrument recording. We will be using an established dataset of audio files such as NSynth to train a convolutional neural network to fulfill this goal. This dataset also contains classifiers such as "acoustic", "synthetic", and "electronic", as well as "attack" and "decay" time. These are all useful features to have when describing a snippet of audio. NSynth already has its audio files encoded down to integer arrays, and comes with a wide featureset that we will be able to use for training. We will be using libraries such as scikit-learn library to train and test our model, as well as Librosa to extract features from the dataset if we need to.'
